...
import { openConfirm, openCatModal, openCatConfig, getCatEditId, closeCatModal, getCatCfgId, closeCatConfig } from '../ui/modals.js';
import { openSociosList } from './socios.js';

export async function loadCategorias(){
  const supabase = getClient();
  const grid=$('#socGrid'); grid.innerHTML='<div class="loading">Cargando categorías…</div>';
    ...
    window.Sortable.create(grid, {
      ...
      onEnd: async function(){
        try{
          ...
          await Promise.all(items.map(it => supabase.from('categorias_socios').update({orden: it.orden}).eq('id', it.id)));
...
export function buildCatCard(row){
  const bg=row.color||'#3ba55d', txt=contrastColor(bg), brd=borderOn(bg);
  const card=el('div',{class:'card',style:{background:bg,borderColor:brd,color:txt}},[]);
  card.setAttribute('data-id',String(row.id||''));

  const titleWrap=el('div',null,[]);
  const title=el('div',{style:{display:'flex',alignItems:'center',gap:'8px',fontWeight:'700',fontSize:'16px',paddingRight:'70px'}},[]);
  title.appendChild(el('span',{class:'color-dot',style:{background:bg}}));
  title.appendChild(el('span',null,[String(row.nombre||'—')]));
  const bal=el('div',{style:{marginTop:'8px',fontSize:'15px',fontWeight:'600'}},['Balance: $'+fmt(row.balance||0)]);
  titleWrap.appendChild(title); titleWrap.appendChild(bal);

  const actions=el('div',{class:'actions-column'},[]);
  const ebtn=el('button',{class:'icon-btn edit',title:'Editar',type:'button'},[]); ebtn.innerHTML=pencil();
  const dbtn=el('button',{class:'icon-btn delete',title:'Eliminar',type:'button'},[]); dbtn.innerHTML=trash();
  const cbtn=el('button',{class:'icon-btn config',title:'Configuración',type:'button'},[]); cbtn.innerHTML=gear();

  ebtn.addEventListener('click', ev=>{ ev.stopPropagation(); openCatModal('edit', row); });
  dbtn.addEventListener('click', async ev=>{
    ev.stopPropagation();
    openConfirm('¿Eliminar esta categoría? Esta acción no se puede deshacer.', async ()=>{
      const supabase = getClient();
      const {error} = await supabase.from('categorias_socios').delete().eq('id', row.id);
      if(error) return alert(error.message);
      loadCategorias();
    }, 'Eliminar categoría');
  });
  cbtn.addEventListener('click', ev=>{ ev.stopPropagation(); openCatConfig(row); });

  actions.appendChild(ebtn); actions.appendChild(dbtn); actions.appendChild(cbtn);
