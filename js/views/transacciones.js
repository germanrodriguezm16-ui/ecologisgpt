...
export function openTransaccionesView() {
  ...
  try {
    ...
    console.error('[Transacciones] error creating FAB', e);
  }
...
export async function prepareTransaccionModal() {
  ...
  try {
    ...
    destinoSel?.addEventListener('change', () => {
      if (origenSel && destinoSel && origenSel.value === destinoSel.value)
        alert('Origen y destino no pueden ser el mismo socio');
    });
    ...
    function syncFechaDisplay() {
      if (fechaDisplay)
        fechaDisplay.textContent = formatFechaDisplayFromDatetimeLocal(fechaInput.value);
    }
...
export async function renderTransacciones() {
  ...
  thead.innerHTML =
    '<tr><th>Fecha</th><th>Origen</th><th>Destino</th><th>Valor</th><th>Comentario</th></tr>';
  ...
  r.rows.forEach(t => {
    ...
    tr.innerHTML =
      ...
      '</td>' +
      '<td>' +
      esc(String(t.origen_socio_id || t.origen_categoria_id || '')) +
      '</td>' +
      '<td>' +
      esc(String(t.destino_socio_id || t.destino_categoria_id || '')) +
      '</td>' +
      '<td>$' +
      esc(String(t.valor || 0)) +
      '</td>' +
      '<td>' +
      esc(String(t.comentario || '')) +
      '</td>';
...
export async function handleTransaccionFormSubmit(e) {
  ...
  try {
    ...
    const { error } = await supabase.rpc('insert_transaccion_and_update_balances', payload);
