...
export function createCurrencyFSM(){
  ...
  function inputDigit(d){
    ...
    if (!hasDec){
      if (intPart === '0') intPart = d; else intPart = intPart + d;
    } else {
      if (decPart.length < 2) decPart += d;
    }
  }

  function inputSep(){ if (!hasDec) hasDec = true; }

  function backspace(){
    if (hasDec && decPart.length > 0){ decPart = decPart.slice(0,-1); return; }
    if (hasDec && decPart.length === 0){ hasDec = false; return; }
    // borrar entero
    if (intPart.length > 1) intPart = intPart.slice(0,-1); else intPart = '0';
  }

  function reset(){ intPart = '0'; decPart = ''; hasDec = false; }

  function pad2(){
    if (!hasDec){ decPart = '00'; hasDec = true; }
