...
function candidateElements(){
  ...
  for(const el of all){
    try{
      ...
      candidates.push(el);
    }catch(e){ /* ignore errors on exotic elements */ }
  }
  return candidates;
}
...
function restoreMainIfHidden(){
  ...
  [body, layout, main].forEach(el=>{
    ...
    try{
      ...
      if(modified) changed.push({selector: selectorFor(el), before, after: {display: el.style.display, visibility: el.style.visibility, opacity: el.style.opacity}});
    }catch(e){}
  });
  return changed;
}
